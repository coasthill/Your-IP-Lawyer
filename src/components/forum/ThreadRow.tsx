...
export function ThreadRow({ thread, compact = false }: { thread: ThreadPublic; compact?: boolean }) {
  ...
  return (
    <article
      className={cn("group relative grid gap-x-8 gap-y-3 border-b border-bronze/15 md:grid-cols-[minmax(0,1fr)_auto] md:items-start", compact ? "py-4" : "py-6 md:py-7")}
      aria-labelledby={titleId}
    >
      <div className="min-w-0">
        {hasMarkers ? (
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
            {thread.pinned ? <span className="font-mono text-[0.6rem] uppercase tracking-[0.22em] text-bronze-2">Pinned</span> : null}
            {thread.category ? (
              <Link
                href={forumHref({ category: thread.category.slug })}
                className="chip relative z-[1] whitespace-nowrap transition-colors hover:border-current hover:text-ivory"
              >
                {thread.category.name}
              </Link>
            ) : null}
            {thread.locked ? <span className="font-mono text-[0.6rem] uppercase tracking-[0.22em] text-ash">Closed</span> : null}
          </div>
        ) : null}
        ...
        <h3
          id={titleId}
          className={cn(
            "font-display leading-[1.15] text-ivory transition-colors duration-300 group-hover:text-bronze-2 group-focus-within:text-bronze-2",
        ...
        <p className="mt-2 font-mono text-[0.62rem] uppercase tracking-[0.18em] text-bone/70">
        ...
        {!compact && thread.tags.length ? (
          <ul className="mt-3 flex flex-wrap gap-1.5" aria-label="Tags">
            {thread.tags.map((t) => (
              <li key={t.id}>
                <span className="chip text-[0.58rem] text-bone/70">{t.name}</span>
              </li>
            ))}
          </ul>
        ) : null}
      ...
      <dl
        className={cn(
          "flex flex-wrap gap-x-5 gap-y-1 font-mono text-[0.62rem] uppercase tracking-[0.18em] text-ash md:flex-col md:items-end md:gap-y-1.5 md:text-right",
          compact ? "md:pt-1" : "md:pt-1.5",
        )}
      >
        <div>
          <dt className="sr-only">Replies</dt>
          <dd className="text-parchment">{pluralise(thread.replyCount, "reply", "replies")}</dd>
        </div>
        <div>
