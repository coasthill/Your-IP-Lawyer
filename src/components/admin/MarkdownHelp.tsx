...
export function MarkdownHelp() {
  return (
    <details className="group border">
      <summary className="cursor-pointer list-none px-4 py-3 font-mono text-[0.64rem] uppercase tracking-[0.18em] text-slate transition-colors hover:text-ink [&::-webkit-details-marker]:hidden">
      ...
      <div className="border-t px-4 py-4">
        <p className="mb-3 text-xs text-slate">Articles are written in Markdown. Footnotes, tables, block quotes and images are supported; raw HTML is stripped for safety.</p>
        <dl className="grid gap-x-6 gap-y-2 text-xs sm:grid-cols-[auto_1fr]">
          {ROWS.map(([syntax, meaning]) => (
            <div key={syntax} className="contents">
              <dt>
                <code className="whitespace-pre-wrap font-mono text-[0.7rem] text-lapis">{syntax}</code>
              </dt>
