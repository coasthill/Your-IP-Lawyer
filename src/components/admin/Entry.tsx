...
export function EntryList({ children, className }: { children: ReactNode; className?: string }) {
  return <ol className={cn("divide-y border-y", className)}>{children}</ol>;
}
  ...
  return (
    <li id={id} className="grid gap-3 py-5 md:grid-cols-[minmax(0,1fr)_auto] md:gap-6">
      <div className="min-w-0">
        ...
        {title ? <p className="mt-2 font-display text-xl leading-snug text-ink">{title}</p> : null}
        {bodyHtml ? <div className="prose-ugc mt-2 max-w-prose text-charcoal" dangerouslySetInnerHTML={{ __html: bodyHtml }} /> : null}
        {footer ? <div className="mt-2 text-xs text-slate">{footer}</div> : null}
      </div>
