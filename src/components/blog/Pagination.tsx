...
export function Pagination({ basePath, page, hasNext }: { basePath: string; page: number; hasNext: boolean }) {
  ...
  return (
    <nav aria-label="Pagination" className="mt-16 flex items-center justify-between gap-4 border-t border-current/15 pt-8">
      <div>
        {page > 1 ? (
      ...
      <p className="font-mono text-[0.62rem] uppercase tracking-[0.2em] text-current/60">Page {page}</p>
      <div>
        {hasNext ? (
