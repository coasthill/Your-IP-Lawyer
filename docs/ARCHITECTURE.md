...
**`frameAt(p, frameCount)`** resolves progress into `{ a, b, mix, kind }`. `a` and `b` are `FrameRef`s — `{ beat, frame, fraction, drift }` — with `b` null outside a transition window and `mix` a smoothstep across it; during the window both beats keep playing (A on its last frames, B on its first), so the film never stalls. `frameCount(clipId)` — supplied by `art.ts` as `clipFrameCount` — turns the 0–1 `fraction` into a frame index (`round(fraction · (n − 1))`), 0 when the clip has no frames on disk. Within a clip beat the progress-within-beat `q` first passes through `hold` (`[a, b]`: the clip finishes at `q = a` and freezes on its last frame — the assembly uses `[0.85, 1]`) and then through `keyframes` (piecewise-linear `[q, fraction]` pairs, default `[[0, 0], [1, 1]]`). That is how the strike beat pins the clip's impact frame to the scroll: `[[0, 0], [(GAVEL_STRIKE_AT − start) / (end − start), 0.74], [1, 1]]` lands the frame 74 % through the clip — `round(0.74 · 49) = 36`, i.e. `f037.webp`, the moment of contact — on `GAVEL_STRIKE_AT`, where the shader flash and `sound.gavel()` are keyed; a new strike clip means a new fraction there (impact frame's 0-based index ÷ (frames − 1)). Within a still beat `driftAt` eases `drift.from → drift.to` (scale, pan x, pan y) with smoothstep; a clip has identity drift unless it declares one.

Seven of the ten beats are clips today; `orchard` and `structure` are still beats even though their clips are sliced and committed under `public/art/film/` — switching either is a one-line change of the beat's `media` in `FILM`.
