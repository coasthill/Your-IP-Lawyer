...
The homepage is a short **film** in the style of an oil painting — ten scenes, or *beats*, that the visitor moves through by scrolling. Nine beats are **clips** (five-second videos made with Kling on Higgsfield, upscaled to 2K, encoded into web video by the robot, and played as real video while the beat is on screen), one is a **painting** under a slow camera drift, and the scroll carries the visitor from each beat to the next through a ripple, a curtain or a dissolve. Every clip was generated *from* one of the paintings, so the two sets match. Each painting and each clip has a short name (its `id`) that is used everywhere:
...
| 2 | The gown fills | the black silk billows; gold emblems surface in the folds and sink back (plays as a seamless loop) | `gown-fills` | `gown` |
| 3 | The strike | the gavel hangs a beat too long, then strikes | `strike` | `gavel` |
| 4 | The orchard | a tree whose fruit are the emblems of IP; the breeze stirs the leaves and the emblems light up (a seamless loop) | `orchard` | `orchard` |
| 5 | The disclosure | a glowing idea in cupped hands splits open to reveal a drawing | `disclosure` | `idea` |
| 6 | The structure | sculptors carve a column crowned with the scales of justice; the mallets tap, the scales sway and settle (a seamless loop) | `structure` | `structure` |
...
Three older paintings — `patent` (the brass mechanism), `design` (the vase) and `gi` (the still life) — stay in the set as **stand-ins**: if a clip's video files are ever missing (the robot has not run yet, or its run failed), or while a clip is still loading, or when a browser refuses to play it, the site shows a painting in that beat instead, so the homepage never breaks. The stand-ins are `advocate` for the assembly, `gown` for the gown, `gavel` for the strike, `orchard` (then `patent`) for the orchard, `patent` for the disclosure, `structure` (then `design`) for the structure, `trademark` for the certificate, `gi` for the sapling and `water-still` for the water. Three clips — `gown-fills`, `orchard` and `structure` — are encoded as seamless **loops** (the video runs forward, then backwards, and repeats) so those beats never freeze however long the visitor lingers; the other clips play once and hold their last picture.

**Where things live**

- `content/artwork/manifest.json` — the list of the thirteen paintings (`scenes`: `id`, `source`, `focal`) and the nine clips (`clips`: `id`, `source` — the web address of the video — `reverse`, and three optional settings: `trim` — two numbers in seconds to keep only part of a video, no clip uses it today; `loop` — `true` on `gown-fills`, `orchard` and `structure`, which makes the robot encode a seamless forward-then-backward loop; and `impactAt` — only on `strike`, the second at which the gavel meets the block). An old `fps` field, if you still see one, is ignored. **This is the only file you edit to change a painting or a clip.**
...
- `public/art/film/<id>/` — one folder per clip: the video the site plays, in two formats (`clip.webm` and `clip.mp4`, up to 2560 pixels wide) and two phone-sized copies (`clip-sm.webm` and `clip-sm.mp4`, up to 1280 pixels wide); `poster.webp` and `poster-sm.webp`, the clip's first picture, shown until the video is ready; and `sheet.jpg`, a strip of five frames for checking the clip at a glance. Also made by the robot.
- `src/components/hero/art-manifest.json` — sizes, focal points, clip lengths, the gavel's impact time, the paths of the video files and tiny blurred placeholders. Also written by the robot.
...
**A word about credits before you generate anything.** The account is on the basic Higgsfield plan. A painting costs about **2 credits** (GPT Image 2, 3:2, 2K, medium quality). A clip costs about **6.25 credits** (Kling 3.0, model `kling3_0`, *std* mode, sound off, 5 seconds, 16:9) — three paintings' worth — and the *pro* mode needs a Plus plan. Every clip is then **upscaled** on Higgsfield (ByteDance video upscale, preset `aigc`, 2K, 24 fps) from 1176 × 780 to 2168 × 1440 for about **0.2 credits** more; the basic plan runs at most two upscale jobs at a time. Generate **one thing at a time**, look at it, and only then decide whether to try again. **Never generate video for anything other than these nine clips** — no other page uses video, and a few unnecessary clips can empty the account.
...
6. Open the **Actions** tab. The workflow **"Fetch artwork"** starts by itself whenever `manifest.json` changes, on any branch. Wait for the green tick (a minute or two). It downloads the picture, converts it to the two web sizes, updates `art-manifest.json` and **commits the result on the same branch** with the message "Artwork: fetch and encode film clips" (the same message whatever changed — a painting, a clip or both). You do not need to commit anything else.
...
4. **Upscale it.** Kling delivers 1176 × 780 pixels, which looks soft on a large or retina screen. In Higgsfield choose **Video upscale** (ByteDance), preset **aigc**, **2K**, **24 fps**, and upscale the clip you kept — about 0.2 credits, and the basic plan runs at most two upscale jobs at a time. The result is 2168 × 1440.
5. In your Higgsfield **library**, **copy the video URL of the upscaled clip** (a long address ending in `.mp4`).
6. On GitHub, open `content/artwork/manifest.json`, find the clip under `clips` and paste the address as its `source`. Leave the `id` alone. Set `"reverse": true` **only** on the assembly clip (that is the backwards trick); every other clip has `"reverse": false`. Keep `"loop": true` on `gown-fills`, `orchard` and `structure` (the robot turns them into seamless loops) and keep `impactAt` on the strike clip (see "The gavel's timing" below — a new strike clip needs a new number there). If an old `"fps"` line is still in the file it does no harm; the robot ignores it.
7. **Commit changes** and wait for the **Fetch artwork** run (encoding a clip takes a few minutes longer than converting a painting). The robot downloads the video, encodes it into the two formats at two sizes, writes the posters and the contact sheet, updates `art-manifest.json` and commits the lot on the same branch. A clip whose address and settings have not changed is left alone on later runs.
8. Look at the contact sheet: open `public/art/film/<id>/sheet.jpg` on GitHub. It shows five frames of the clip *as the site will play it* — start, quarter, half, three-quarters, end (for a loop clip that is the whole forward-and-back cycle). For the assembly clip the sheet should read left to right as dust → advocate. If it reads wrong, fix the prompt or the `reverse` flag and go again.
9. Redeploy (or wait for Vercel's automatic deploy of the robot's commit).

**The gavel's timing.** The flash and the sound follow the clip itself: they fire at the moment the strike video reaches `impactAt` — a number of seconds in `content/artwork/manifest.json`, counted on the clip as Higgsfield made it (before any `trim` or `reverse`), currently `3.6`. The caption's timing on the scroll (`GAVEL_STRIKE_AT` in `story.ts`) is separate and does not change. A new strike clip will land at a different moment. Play the upscaled clip in any video player and note the second at which the gavel makes contact (the contact sheet gets you close; then step through the video around that point), write it into `impactAt` in the manifest and commit — the robot carries it into `art-manifest.json`. Everything about a clip is settled in the manifest; no code changes.

**If the workflow fails** (a red cross in the Actions tab): open the run and read the log. Nine times out of ten the cause is a bad URL — a `404` or `403` in the log means the address was copied incompletely or has expired. Fix the `source` in `manifest.json` and commit again. A message about `ffmpeg` means the video could not be encoded — usually the address is not a direct link to the `.mp4`. Nothing is broken on the live site in the meantime: the old pictures and clips stay until a run succeeds. If the log complains about `sharp` or Node rather than a URL, something else changed in the repository — ask for help rather than re-running.

**Other things worth knowing**

- **Running the workflow by hand:** Actions tab → **Fetch artwork** → **Run workflow** (the button appears once the workflow file is on the default branch). Its "candidates" box also converts the addresses in `content/artwork/candidates.json` into small review copies under `public/art/candidates/` — a way to compare a few tries side by side without touching the live scenes. Delete candidates once you have chosen. Its "reencode" box makes the robot encode **every** clip again even when nothing changed — only needed after the encoder settings in `scripts/fetch-artwork.mjs` change, or to replace stand-in video files; it takes a while and rewrites about 90 MB.
- **Running it on your computer:** after `npm install`, run `node scripts/fetch-artwork.mjs` (the image tool it needs, `sharp`, is already installed with the site). Paintings need nothing else; encoding clips needs **ffmpeg** installed on your computer — without it the clips are skipped with a message and only the paintings are converted. It writes the same files; commit them yourself.
- **Size:** the video files are committed to the repository by the workflow — roughly **5 to 20 MB per clip** (the two formats at two sizes, the posters and the sheet; the loop clips are the largest because they run twice as long), about 90 MB for all nine — and every replacement adds another copy to the repository's history. A visitor never downloads all of it: the site fetches only the beat on screen and the next one, and phones get the smaller files. That is fine at this scale; it is one more reason not to generate clips you do not need.
...
- **The homepage shows a plain cross-fade (or the still version) on my laptop** — the site uses the WebGL version wherever WebGL works and is not software-rendered; add `?render=webgl` to the address to force it, `?render=canvas` for the 2D cross-fade, `?render=static` for the still version. (`&snap` additionally removes the scroll easing, which is useful for screenshots; `&t=4` then freezes the clip on screen at that many seconds, so a screenshot can show it mid-motion.)
- **The clips do not move — I only see a still picture in each scene** — the browser has declined to play the video: a phone in Low Power Mode or with a data saver switched on does this, and so does `?video=0` in the address (a way to see the posters). The film still works, with the clip's first picture in place of the video; on a phone, one tap on the page usually lets the clips play. If the picture in a scene is one of the older paintings (the brass mechanism, the vase, the still life), that clip's files are missing — check the Fetch artwork run.
