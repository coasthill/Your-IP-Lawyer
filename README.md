...
- `content/artwork/manifest.json` — the list of the thirteen paintings (`scenes`: `id`, `source`, `focal`) and the nine clips (`clips`: `id`, `source` — the web address of the video — `fps`, always 10, `reverse`, and an optional `trim` — two numbers in seconds to keep only part of a video; no clip uses it today). **This is the only file you edit to change a painting or a clip.**
...
**The gavel's timing.** The film pins the moment of impact to a fixed point of the scroll (`GAVEL_STRIKE_AT` in `story.ts` — the flash and the sound happen there, and the caption reads just after it). For that to work the strike beat records *where in the clip* the gavel lands: the `keyframes` on the `strike` beat in `story.ts`, currently `0.74` (74 % of the way through the clip — frame `f037.webp` of the present clip, the moment of contact; `f038` is already the dust). A new strike clip will land at a different moment. Find the frame where the gavel makes contact (in `public/art/film/strike/` — 50 frames make 5 seconds, ten per second; the contact sheet gets you close, then look at the frames around it), subtract one from its number and divide by 49 (f037 → 36 ÷ 49 ≈ 0.74), and ask for that number to be put into the keyframe. Everything else about a clip needs no code change.
